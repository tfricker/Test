...
UDTExamples.df <- UnderpredValues.df[c(14, 21, 36, 63, 80),] %>%
  mutate(Names = c("Spencer, SD", "Camilla, GA", "Camilla, GA", "Smithville, MS/Shottsville, AL", "Garland-Rowlett, TX"))
...
ggplot(predValues.df[predValues.df$Actual >= 25, ], aes(x = Actual, y = Estimate)) +
  ...
  geom_text_repel(data = UDTExamples.df, aes(label = Names), size = 2.5, nudge_x = .08) +
  geom_abline(slope = 1) + 
#  geom_smooth(method = lm, se = FALSE) +
  xlab("Observed Number of Casualties") +
  ylab("Predicted Rate") +

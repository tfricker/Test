...
tm_shape(stateBorders, projection ="+init=epsg:2163") +
  ...
  tm_bubbles("Diff", col = "#1b7837", sizes.legend = c(50, 100, 500, 1000, 1500), alpha = 0.4, title.size="Underprediction", scale = 2.5) + 
...
Map of North America and US Casualty-producing tornadoes
```{r}
data("World")

KT.sfdf <- st_read("/Users/tylerfricker/Desktop/Git Projects/Casualty-Risk-Model/killettor")

NorthAmerica = World %>%
  filter(iso_a3 %in% c("USA", "MEX", "CAN"))

NorthAmerica = st_transform(NorthAmerica, crs = 4121)

tm_shape(NorthAmerica) +
  tm_polygons(col = "grey94") +
  #tm_scale_bar(position = c("RIGHT", "BOTTOM")) +
  #tm_compass(position = c("right", "bottom")) +
  tm_layout(bg.color="white", attr.outside=TRUE)

states <- usa_sf("longlat")
Region <- states %>%
  filter(iso_3166_2 %in% c("Fl", "GA", "AL", "MS", "SC", "TN", "NC", "TX", "OK", "KS", "NE", "SD", "IA", "WI", "MO", "AR", "LA", "OH", "WV", "MI", "IL"))
...
```{r}
library("ggmap")
register_google(key = "AIzaSyAg0GAbWcs0oWZWOMtBzoFWKPxVCU-vquQ")
# must add billing to the API

camilla <- c(left = -84.6486, bottom = 30.8658, right = -83.7571, top = 31.6447)

map = get_googlemap("camilla georgia", zoom = 13, maptype = "hybrid")
...
Now add the tornadoes over the map
```{r}
Cam = Tor.sfdf[633,]
...
ggmap(map) + 
  ...
  geom_sf(data = cam_3857$geometry, inherit.aes = FALSE, alpha =.4) +
  geom_sf(data = cam2_3857$geometry, inherit.aes = FALSE, alpha =.4) +
  labs(x = NULL, y = NULL) +
  theme(legend.position = "none",
        axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
ggmap(map) +
  labs(x = NULL, y = NULL) +
  geom_point(aes(x = -84.198, y = 31.219), color = "red", size = 2) +
  geom_label(aes(label = "Camilla", x = -84.198, y = 31.219),
             hjust = -.15,
             vjust = .25,
             label.size = 0) +
  theme(legend.position = "none",
...
camilla <- c(left = -84.6486, bottom = 30.88, right = -83.7571, top = 31.6)
...
tm_shape(Camilla) +
  tm_dots(col = "#1b7837", size = .75, alpha = 0.6) +
tm_scale_bar(position = c("RIGHT", "BOTTOM")) +
#tm_compass(position = c("right", "bottom")) +
tm_layout(bg.color="white", attr.outside=TRUE)
```
...
tm_shape(MT) +
  ...
  tm_text(text = "iso_3166_2", xmod = 1.25) +
tm_shape(Spencer) +
  tm_dots(col = "#1b7837", size = .7, alpha = 0.6) +
tm_scale_bar(position = c("RIGHT", "BOTTOM")) +
...
tm_shape(Garland) +
  tm_dots(col = "#1b7837", size = 2, alpha = 0.6) +
#tm_shape(Rowlett) +

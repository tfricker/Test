...
Given the large disparity between the mean and variance of casualties the casualty counts are over-dispersed relative to a Poisson distribution. This suggests that a negative binomial distribution may be better.

The number of tornadoes and the number of casualties conditional on at least one mobile home. 
```{r}
as.data.frame(TorSC.sfdf) %>%
  group_by(MoblHms >= 1) %>%
  summarise(nC = sum(cas),
            nT = n())
```
About 13% of all tornado casualties occurred in regions estimated to have at least one mobile home.

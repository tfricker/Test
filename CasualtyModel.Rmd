...
Given the large disparity between the mean and variance of casualties the casualty counts are over-dispersed relative to a Poisson distribution. This suggests that a negative binomial distribution may be the best fit.

The number of tornadoes and the number of casualties conditional on at least one mobile home. 
```{r}
as.data.frame(Tor.sfdf) %>%
  group_by(MoblHms >= 1) %>%
  dplyr::summarize(nC = sum(cas),
            nT = n())
```
...
ggplot(dfB, aes(x = MonthF, y = nCas)) +
...
```{r}
library(MASS)
formula1 <- cas ~ yr + lpopD + lED + lpopD:lED + smh
formula2 <- lcas ~ yr + lpopD + lED + lpopD:lED + smh
...
formula4 <- lcas ~ yr + lpopD + lED + lpopD:lED + smh + (1|mo) + (1|hr)
...
formula5 <- cas | trunc(lb = 1) ~ yr + lpopD + lED + lpopD:lED + smh + (1|hr) + (1|mo)

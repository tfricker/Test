...
Import .shp of the social correlates. Add an hour column into the dataset and rearrange the data.
```{r}
...
TorSC.sfdf$hr = hour(TorSC.sfdf$DateTim)
TorSC.sfdf = TorSC.sfdf[c(1,2,3,4,51,5:50)]
```

Import .csv of the social correlates.
```{r}
...
Given the large disparity between the mean and variance of casualties, it appears the data is overdispersed. This suggests that a negative binomial distribution may be best.

Make the dataframe
```{r}
df = Torn.df %>%
  filter(MobileHomes >= 1) %>%
  mutate(lpopD = log(popD2),
            led = log(ED),
            mh = scale(MobileHomes),
            mh2 = log(MobileHomes),
            mi = log(MedianIncome),
            yr = Year)
```
...
Create the model. ModelFE is a fixed effects model. ModelNB is a mixed effect, negative bunomial model, and ModelME is a mixed effects model.
```{r}
formula = cas ~ lpopD + led + lpopD:led + mh2 + yr

modelFE = glm.nb(formula, data = sfdf, link = log)

modelNB = lmer(cas ~ lpopD + led + lpopD:led + mh + yr + (1|mo) + (1|hr), data = sfdf, family = negative.binomial())

modelME = lmer(log(cas) ~ lpopD + led + lpopD:led + mh + yr + (1|mo) + (1|hr), data = sfdf)
...
```{r}
formula = cas | trunc(lb = 1) ~ lpopD + led + lpopD:led + mh2 + yr + (1|mo) + (1|hr)

#formula = cas ~ lpopD + led + lpopD:led + mh + yr + (1|mo)

#family = brms::negbinomial()
family = brms::lognormal()

modelS = brm(family = family,
            formula = formula,
            data = sfdf,
